...
# boost vs. eta weighed by energy efficiency
...
H_gammaEtaCLUE_EB = TH2F("Boost vs. Eta Cluster Energies", ";#gamma factor: Higgs Eta", n_bins_x, np.array(bin_edges_x, dtype=np.float64), n_bins_y, y_min, y_max)
H_gammaEtaPAT_EB = TH2F("Boost vs. Eta PAT Energies", ";#gamma factor: Higgs Eta", n_bins_x, np.array(bin_edges_x, dtype=np.float64), n_bins_y, y_min, y_max)
H_gammaEtaCLUE_EB.SetStats(0)
H_gammaEtaPAT_EB.SetStats(0)
...
H_gammaEtaCLUE_EB.Divide(H_gammaEtaPAT_EB)
...
C.SetLogx()
# C.SetLogy()
H_gammaEtaCLUE_EB.SetTitle("Energy Efficiency (Barrel)")
H_gammaEtaCLUE_EB.Draw("hist")
H_gammaEtaCLUE_EB.Draw("colztext")
xAxis = H_gammaEtaCLUE_EB.GetXaxis()
xAxis.SetTitle("#gamma Factor")
xAxis.CenterTitle(kTRUE)
yAxis = H_gammaEtaCLUE_EB.GetYaxis()
yAxis.SetTitle("Higgs Eta (GeV)")
yAxis.CenterTitle(kTRUE)
C.Print("EfficiencyBoostEtadc2rhoc15delt3.root")