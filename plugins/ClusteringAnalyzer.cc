...
void ClusteringAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
 ...
 // PdgId for photon is 22, final state particles have status == 1
  ...
  if (v_eta.size()>0)   {v_clusterID_EB  = mainRunEB(v_eta,   v_phi,   v_layer,     v_weight,     dc_EB, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_pEE.size()>0) {v_clusterID_EEp = mainRunEE(v_x_pEE, v_y_pEE, v_layer_pEE, v_weight_pEE, dc_EE, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_mEE.size()>0) {v_clusterID_EEm = mainRunEE(v_x_mEE, v_y_mEE, v_layer_mEE, v_weight_mEE, dc_EE, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_pES1.size()>0) {v_clusterID_ES1p = mainRunES(v_x_pES1, v_y_pES1, v_layer_pES1, v_weight_pES1, dc_ES, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_mES1.size()>0) {v_clusterID_ES1m = mainRunES(v_x_mES1, v_y_mES1, v_layer_mES1, v_weight_mES1, dc_ES, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_pES2.size()>0) {v_clusterID_ES2p = mainRunES(v_x_pES2, v_y_pES2, v_layer_pES2, v_weight_pES2, dc_ES, rhoc, outlierDeltaFactor, 0, 10, 0);}
  if (v_x_mES2.size()>0) {v_clusterID_ES2m = mainRunES(v_x_mES2, v_y_mES2, v_layer_mES2, v_weight_mES2, dc_ES, rhoc, outlierDeltaFactor, 0, 10, 0);}
